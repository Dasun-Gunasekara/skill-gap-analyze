...
export default function RoleDetails() {
  ...
  const fetchRoleDetails = async () => {
    try {
      const res = await API.get(`/user/role/${id}`);
      setRole(res.data);
    } catch (error) {
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <Card className="glass-panel">
        <Title level={2} style={{ marginTop: 0, color: 'var(--primary)' }}>Role Details</Title>
            ...
            renderItem={skill => (
              <List.Item style={{ borderColor: 'var(--border)' }}>
                <Typography.Text style={{ color: 'var(--text)', fontSize: '1.1rem' }}>
                  <CheckCircleOutlined style={{ color: 'var(--secondary)', marginRight: 12 }} />
                  {skill}
                </Typography.Text>
              </List.Item>
            )}
